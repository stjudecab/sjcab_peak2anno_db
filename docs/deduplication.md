...
`dedup-genebed` selects one transcript per gene and falls back to the longest
transcript when no selector matches. `filter-genebed` uses the same selectors but
omits genes without a selector match.

`dedup-feature` and `filter-feature` have the same options and selector methods
as their `*-genebed` counterparts. They keep FeatureBED elements only for the
transcript IDs retained by the corresponding GeneBED selection.

```bash
sjcab-peak2anno-db dedup-genebed hg38 v31 -m longcol5 -b all.gene.bed -o annotations
...
sjcab-peak2anno-db filter-genebed -m perover -i active_chromhmm.bed -o annotations
sjcab-peak2anno-db dedup-feature hg38 v31 -m longcol5 -o annotations
sjcab-peak2anno-db filter-feature hg38 v31 -m isoID -i isoforms.txt -o annotations
```

## Selector methods

- `longcol5`: largest numeric BED column 5; the default for `dedup-genebed`.
